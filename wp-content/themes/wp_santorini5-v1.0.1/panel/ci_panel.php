...
function ci_register_theme_default_scripts()
{
	...
	wp_register_script('jquery-superfish', get_child_or_parent_file_uri('/panel/scripts/superfish-1.7.4.min.js'), array('jquery', 'jquery-hoverIntent'), '1.7.4', true);
	wp_register_script('jquery-fitVids', get_child_or_parent_file_uri('/panel/scripts/jquery.fitvid-1.0.js'), array('jquery'), '1.0', false);
...
function ci_enqueue_admin_scripts() 
{
	...
	if($pagenow=='themes.php' and isset($_GET['page']) and $_GET['page']=='ci_panel.php')
	{
		...
		{
			$css = '.tab input[type=checkbox] + label, .tab input[type=radio] + label { position:relative; top: 0px; margin-left: 5px; }
					.set input[type=radio] + label { margin-bottom: 7px; }
					.tab input.radio, .tab input.check { margin-right: 5px; }';
			wp_add_inline_style('ci-panel-css', $css);
		}
	}
...
function ci_panel_upload_image($fieldname, $label)
{
	...
	<div class="up-preview"><?php echo (!empty($ci[$fieldname]) ? '<img src="'.esc_attr($ci[$fieldname]).'" />' : '' );  ?></div>
	<?php if( !empty($ci[$fieldname]) ): ?>
		<div class="up-preview-bg group">
			...
			<span class="swatch transparent" title="<?php echo esc_attr( sprintf(_x('Preview in %s background', 'preview in color background', 'ci_theme'), _x('Transparent', 'color', 'ci_theme')) ); ?>"></span></div>
	<?php endif; ?>
	<?php
}
...
function ci_panel_terms_checklist($fieldname, $taxonomy, $label)
{
	global $ci;
	?>
	<label for="ul-<?php echo $fieldname; ?>"><?php echo $label; ?></label>
	<ul id="ul-<?php echo $fieldname; ?>" class="terms_checklist">
		<?php
			$cats = $ci[$fieldname];
			...
			wp_terms_checklist( 0, array(
				...
				'checked_ontop' => false,
				'taxonomy' => $taxonomy,
				'walker' => new CI_Panel_Walker_Category_Checklist($fieldname)
			) );
		?>
	</ul>
	<?php
...
class CI_Panel_Walker_Category_Checklist extends Walker {
	var $field_name = ''; // This will be used as base for the HTML name="" attributes.
	 ...
	 *
	 * Only this function differs from the default Walker_Category_Checklist class (as of WP 3.8.1)
	 *
	 * @see Walker::start_el()
	 *
	 * @since 2.5.1
	...
	function start_el( &$output, $category, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( $taxonomy == 'category' )
			$name = THEME_OPTIONS . '[' . $this->field_name . ']';

		$class = in_array( $category->term_id, $popular_cats ) ? ' class="popular-category"' : '';

		$input_id = 'in-'.$this->field_name.'-'.$taxonomy.'-' . $category->term_id;

		$output .= "\n<li id='{$this->field_name}-{$taxonomy}-{$category->term_id}'$class>";
		//$output .= '<label class="selectit"><input value="' . $category->term_id . '" type="checkbox" name="'.$name.'[]" id="in-'.$this->field_name.'-'.$taxonomy.'-' . $category->term_id . '"' . checked( in_array( $category->term_id, $selected_cats ), true, false ) . disabled( empty( $args['disabled'] ), false, false ) . ' /> ' . esc_html( apply_filters('the_category', $category->name )) . '</label>';
		$output .= '<input class="check" value="' . $category->term_id . '" type="checkbox" name="'.$name.'[]" id="'.$input_id.'"' . checked( in_array( $category->term_id, $selected_cats ), true, false ) . disabled( empty( $args['disabled'] ), false, false ) . ' />';
		$output .= '<label class="selectit" for="'.$input_id.'">' . esc_html( apply_filters('the_category', $category->name )) . '</label>';
