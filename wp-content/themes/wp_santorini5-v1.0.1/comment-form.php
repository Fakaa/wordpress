	...
	?>
		<?php if ( comments_open() ) : ?>
			<?php do_action( 'comment_form_before' ); ?>
				 ...
				 <div class="post-form">
				 <h3 id="reply-title" class="comments section-title"><span><?php comment_form_title( $args['title_reply'], $args['title_reply_to'] ); ?></span></h3>
					...
					<?php else : ?>
						<form action="<?php echo site_url( '/wp-comments-post.php' ); ?>" method="post" id="<?php echo esc_attr( $args['id_form'] ); ?>">
							...
							<p>
								<input type="submit" class="button action-btn" name="submit" id="<?php echo esc_attr( $args['id_submit'] ); ?>" value="<?php echo esc_attr( $args['label_submit'] ); ?>" />
