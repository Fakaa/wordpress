...
//			: get_metadata( $type, $id, $field_id, ( $single || $repeat ) /* If multicheck this can be multiple values */ );
		...
		{
			$data = cmb_Meta_Box::get_option( $id, $field_id );
		}else{
			$data = isset(cmb_Meta_Box::$aWilokeSettings[$field_id]) ? cmb_Meta_Box::$aWilokeSettings[$field_id] : '';
		}
	...
	public function _set_field_defaults( $args ) {
		...
		$args['options']    = 'group' == $args['type'] ? wp_parse_args( $args['options'], array(
			'add_button'    => esc_html__( 'Add Group', 'roark' ),
			'remove_button' => esc_html__( 'Remove Group', 'roark' ),
		) ) : $args['options'];
