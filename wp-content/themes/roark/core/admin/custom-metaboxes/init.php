...
class cmb_Meta_Box {
	 ...
	 * wiloke
	 */
	public static $aWilokeSettings = array();
	...
	public static function autoload_helpers( $class_name ) {
		...
		$file = "$dir/core/admin/custom-metaboxes/helpers/$class_name.php";
		if ( file_exists( $file ) )
			@include( $file );
	}
	...
	public function register_scripts() {
			...
			if ( ! is_admin() ) {
		   	...
		   	wp_register_script( 'wp-color-picker', admin_url( 'js/color-picker.min.js' ), array( 'iris' ), self::CMB_VERSION );
				wp_localize_script( 'wp-color-picker', 'wpColorPickerL10n', array(
					'clear'         => esc_html__( 'Clear', 'roark' ),
					'defaultString' => esc_html__( 'Default', 'roark' ),
					'pick'          => esc_html__( 'Select Color', 'roark' ),
					'current'       => esc_html__( 'Current Color', 'roark' ),
				) );
			}
		} else {
		...
		wp_localize_script( 'cmb-scripts', 'cmb_l10', apply_filters( 'cmb_localized_data', array(
			...
			'up_arrow'        => '[ ↑ ]&nbsp;',
			'down_arrow'      => '&nbsp;[ ↓ ]',
			'check_toggle'    => esc_html__( 'Select / Deselect All', 'roark' ),
		) ) );
	...
	public function post_metabox($post) {
		if ( ! $this->_meta_box )
			return;
		self::$aWilokeSettings = get_post_meta($post->ID, $this->_meta_box['id'], true);
	...
	public static function show_form( $meta_box, $object_id = 0, $object_type = '' ) {
		...
		$jsonDependencyOnTemplate = $cssClass = '';
		if ( isset($meta_box['dependency_on_template']) && !empty($meta_box['dependency_on_template']) )
		{
			$jsonDependencyOnTemplate['operator'] = $meta_box['dependency_on_template'][0];
			$jsonDependencyOnTemplate['match'] 	  = $meta_box['dependency_on_template'][1];

			$jsonDependencyOnTemplate = json_encode($jsonDependencyOnTemplate);
			$cssClass = 'wiloke-table-has-dependency-on-template';
		}

		// Use nonce for verification
		echo "\n<!-- Begin CMB Fields -->\n";
		do_action( 'cmb_before_table', $meta_box, $object_id, $object_type );
		echo '<table class="form-table cmb_metabox '.esc_attr($cssClass).'" data-dependencyontemplate="'.esc_attr($jsonDependencyOnTemplate).'">';

		if ( isset($meta_box['description']) && !empty($meta_box['description']) )
		{
			echo "\n<tr><td></td><td><p class='cmb_metabox_description'>".esc_html($meta_box['description'])."</p></td></tr>\n";
		}
		...
		foreach ( $meta_box['fields'] as $field_args ) {
			...
			// wiloke team: Added parent_name
			$field_args['parent_name'] = $meta_box['id'];
		...
		<tr class="repeatable-grouping" data-iterator="'. $field_group->count() .'">
			<td>
				...
				foreach ( array_values( $field_group->args( 'fields' ) ) as $field_args ) {
					...
					// wiloke add parent_name to group
					$field_args['parent_name'] = $field_group->args('parent_name');
	...
	public static function save_fields( $meta_box, $object_id, $object_type = '' ) {
		...
		// Set/get type
		$object_key  = $object_type ? $object_type	: self::set_mb_type( $meta_box );
		$object_type = self::set_object_type( $object_key );
		...
		if ( isset($_POST[$meta_box['id']]) && !empty($_POST[$meta_box['id']]) )
		{
			$aNewValues = self::wiloke_get_data($meta_box, $_POST[$meta_box['id']]);

			self::save_field( $aNewValues, $object_id, $meta_box['id'] );
		}
	...
	public static function wiloke_get_data(array $aData, $aValues)
	{
		foreach ( $aData['fields'] as $field_args )
		{
			if ( isset($aValues[$field_args['id']]) )
			{

				// Save default fields
				$field 		= new cmb_Meta_Box_field( $field_args );

				$value = self::wiloke_deep_sanitize_data($field_args, $field, $aValues[$field_args['id']]);

				// $value = self::wiloke_deep_sanitize_data($field_args, $field, $aValues);

				if ( $value )
				{
					$aValues[$field_args['id']] = $value;
				}
	 ...
	 * Help array map
	 */
	public static function wiloke_help_for_map($args)
	{
		$aValues 	= $args['avalues'];
		$fields    	= $args['args'];
		
		$field = new cmb_Meta_Box_field($fields);
		return self::wiloke_deep_sanitize_data($aArgs, $field, $aValues);
	}
	 ...
	 * @author wiloke team
	 */
	public static function wiloke_deep_sanitize_data(array $aFieldArgs, $field, $aValues)
	{
		if ( $aFieldArgs['type'] === 'group' )
		{
//			echo '<pre>';
//			var_dump($aFieldArgs['fields']);
//			echo '</pre>';

//			echo '<pre>';
//			var_dump($aValues);
//			echo '</pre>';
			foreach ( $aFieldArgs['fields'] as $key => $aArgs )
			{

				if ( $aArgs['type'] === 'group' )
				{

					if (  isset($aValues[$aArgs['id']]) && !empty($aValues[$aArgs['id']]) )
					{
						$aValues 			= $aValues[$aArgs['id']];
						$nargs['avalues']   = $aValues;
						$nargs['args'] 		= $aFieldArgs['fields'];

						return array_map(
								function ($args) use ($aValues) {
									$field = new cmb_Meta_Box_field($args);
									return self::wiloke_deep_sanitize_data($aArgs, $field, $aValues);
								},
								$aFieldArgs['fields']
						);
						// return array_map(array('cmb_Meta_Box', 'wiloke_help_for_map'), $nargs['args']);
					}
					...
					{
						$aValues[$k] = self::wiloke_handle_group($aFieldArgs['fields'], $aValue);
		...
		}elseif($aFieldArgs['type'] == 'taxonomy_multicheck' ){
			return  $aValues;
		}else{
			return self::sanitize_field( $field, $aValues, false );
		}
	}
	 ...
	 * @author wiloke
	 */
	public static function wiloke_handle_group($aField, $aValue)
	{
		$i = 0;
		foreach ( $aValue as $key => $value )
		{
			if ( $aField[$i]['id'] == $key )
			{
				$field = new cmb_Meta_Box_field($aField[$i]);
				$aValue[$key] = self::sanitize_field($field, $value, false, false);
				$i = $i + 1;
			}else{
				$aValue[$key] = $value;
			}
	...
	public static function sanitize_field( $field,  $aValues = null, $returnArray = true, $isGroup = false )
	{
		$key = $field->id(true);
...
//			if ( is_array($aValues) )
//			{
//				$new_value = isset($aValues[$key]) ? $aValues[$key] : '';
//			}else{
//				$new_value = $aValues;
//			}
//		}else{
...
//		echo '</pre>';
		...
		if ( $returnArray )
		{
			if ( empty($new_value) )
			{
				...
				$aValues[$key] = $field->sanitization_cb( $new_value );
			}
		}else{
			$aValues = $field->sanitization_cb($new_value);
		}
	...
	public static function save_field( $aValues, $postID, $key = '' )
	{
		// wiloke. Change the method save data
		update_post_meta($postID, $key, $aValues);
	}
	...
	public static function get_meta_box_url() {
		...
		if ( strtoupper( substr( PHP_OS, 0, 3 ) ) === 'WIN' ) {
			...
			$content_url = str_replace( $content_dir, WP_CONTENT_URL, get_template_directory_uri() . '/core/admin/custom-metaboxes/' );
		  ...
		  $cmb_url = str_replace(
				...
				get_template_directory_uri() . '/core/admin/custom-metaboxes/'
			);
		}
...
function cmb_metabox_form( $meta_box, $object_id, $echo = true ) {
	...
	$form = sprintf( $form_format, $meta_box['id'], $object_id, $form, esc_html__( 'Save', 'roark' ) );
