	...
	if ( class_exists( 'WPBakeryVisualComposerAbstract' ) ) {
		...
		if ( ! $nm_vcomp_stock ) {
			function nm_vc_frontend_assets() {
				...
				wp_enqueue_style( 'nm_js_composer_front', NM_THEME_URI . '/css/visual-composer/nm-js_composer.css', array(), '1.0', 'all' );
				
				// Enqueue frontend scripts (original file with unused scripts removed)
				wp_enqueue_script( 'nm_composer_front_js', NM_THEME_URI . '/js/visual-composer/nm-js_composer_front.min.js', array( 'jquery' ), '1.0', true );
			}
		...
		function nm_vc_register_shortcodes() {
			...
			if ( nm_woocommerce_activated() ) {
                ...
                include( NM_VC_DIR . '/shortcodes/woocommerce/product-search.php' );
		...
		if ( is_admin() ) {
			...
			function nm_vc_register_elements() {
				...
				if ( nm_woocommerce_activated() ) {
                    ...
                    include( NM_VC_DIR . '/elements/woocommerce/product-search.php' );
					//todo: include( NM_VC_DIR . '/elements/woocommerce/product-slider.php' );
				}
			}
            add_action( 'init', 'nm_vc_register_elements', 2 );
