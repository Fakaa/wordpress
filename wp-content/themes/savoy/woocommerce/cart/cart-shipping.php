...
<li class="shipping">
    ...
    <div class="col-td col-sm-6 col-xs-12" data-title="<?php echo esc_attr( $package_name ); ?>">
        <?php if ( 1 < count( $available_methods ) ) : ?>
			<ul id="shipping_method">
				<?php foreach ( $available_methods as $method ) : ?>
					<li>
						<?php
							printf( '<input type="radio" name="shipping_method[%1$d]" data-index="%1$d" id="shipping_method_%1$d_%2$s" value="%3$s" class="shipping_method nm-custom-radio" %4$s />
								<label for="shipping_method_%1$d_%2$s" class="nm-custom-radio-label">%5$s</label>',
