...
<div class="ezfc wrap ezfc-wrapper container-fluid">
	<div class="row">
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
			<div class="inner">
				<?php echo "<h2 class='heading-title'>" . __("Overview", "ezfc") . " - ez Form Calculator v" . EZFC_VERSION . " <span class='ezfc-loading'><i class='fa fa-cog fa-spin'></i></span></h2>"; ?>
				...
				<div class="ezfc-elements-add ezfc-accordion" id="ezfc-elements-add">
					...
					<div>
						<ul class="ezfc-elements ezfc-cat-steps">
							<?php ezfc_list_elements($elements_cat["steps"], "form_element_add"); ?>
						</ul>
					</div>
	...
	<div class="ezfc-functions-dialog ezfc-default-dialog" id="ezfc-custom-calculation-functions" title="<?php echo __("Custom calculation functions", "ezfc"); ?>">
		<dl>
			...
			$calculation_functions = array(
				...
				"ezfc_functions.get_element_id_by_name(form_id, name)" => __("Get the ID of an element by its name. Example:", "ezfc") . "<br><pre>var element_id = ezfc_functions.get_element_id_by_name(1, 10);</pre>"
			);
...
<div id="ezfc-advanced-actions-dialog" class="ezfc-default-dialog" title="<?php echo __("Advanced actions", "ezfc"); ?>">
	...
	foreach ($advanced_actions as $func => $action) {
		...
		$out[] = "<div class='inner' style='margin-bottom: 10px'>";
		$out[] = "<h5>{$action["title"]}</h5>";
		$out[] = "<p>{$action["description"]}</p>";
		$out[] = "<p><button class='button button-primary' data-{$action["scope"]}='{$func}'>" . __("Execute", "ezfc") . "</button></p>";
		$out[] = "</div>";
	}
...
<div id="ezfc-dialog-batch-edit" class="ezfc-dialog" title="<?php esc_attr(_e("Batch edit", "ezfc")); ?>">
	<p id="ezfc-dialog-batch-edit-description"></p>
	<textarea id="ezfc-batch-edit-textarea"></textarea>
</div>
