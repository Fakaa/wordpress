...
class Ezfc_frontend {
	...
	public function __construct() {
		...
		// load pdf extension
		if (!class_exists("EZFC_Extension_PDF") && version_compare(PHP_VERSION, "5.3.0") >= 0) {
	...
	public function check_input($id, $data_raw, $preview_id=null) {
		...
		foreach ($form_elements as $fe_id => $form_element) {
			...
			if (isset($data[$fe_id]) && is_array($data[$fe_id]) && $el_type == "checkbox") {
				...
				// min/max selected options check
				$selected = count($data[$fe_id]);
				// min
				if (!empty($element_data->min_selectable) && $selected < $element_data->min_selectable) {
					return Ezfc_Functions::send_message("error", sprintf(get_option("ezfc_text_error_min_selectable"), $element_data->min_selectable), $fe_id);
				}
				// max
				if (!empty($element_data->max_selectable) && $selected > $element_data->max_selectable) {
					return Ezfc_Functions::send_message("error", sprintf(get_option("ezfc_text_error_max_selectable"), $element_data->max_selectable), $fe_id);
				}
			}
				...
				if (!$empty) {
					switch ($el_type) {
						...
						case "daterange":
							...
							$days = Ezfc_Functions::count_days_format($datepicker_format, $input_value[0], $input_value[1], $element_data->workdays_only);
							...
							else if ($days > $element_data->maxDays) {
						...
						case "numbers":
							...
							if (!empty($element_data->min)) {
								if ($input_value < $element_data->min) return Ezfc_Functions::send_message("error", __("Minimum value:", "ezfc") . $element_data->min , $fe_id);
							}
							if (!empty($element_data->max)) {
								if ($input_value > $element_data->max) return Ezfc_Functions::send_message("error", __("Maximum value:", "ezfc") . $element_data->max , $fe_id);
							}
						break;
					}
				}
	...
	public function insert($id, $data, $ref_id, $send_mail=true, $payment=array()) {
		...
		**/
		...
		if (!$add_to_cart) {
			...
			if (!$res) return Ezfc_Functions::send_message("error", __("Submission failed.", "ezfc"));
	...
	**/
			...
			foreach ($value as $n => $input_value) {
				...
				if (property_exists($element, "factor") && $value) {
					...
					if ($element_type == "daterange") {
						$datepicker_format = $this->submission_data["options"]["datepicker_format"];
						$days = Ezfc_Functions::count_days_format($datepicker_format, $value[0], $value[1], $element->workdays_only);
						...
						$tmp_total = $this->normalize_value($input_value) * $element->factor;
				...
				if (!empty($element->calculate)) {
					...
					foreach ($element->calculate as $calc_row_open_index => $calc_row_open) {
						if ($calc_row_open->target == "__open__" && !property_exists($calc_row_open, "reference_index")) {
							...
							foreach ($element->calculate as $calc_row_close_index => $calc_row_close) {
					...
					foreach ($element->calculate as $calc_index => $calc_array) {
						...
						if (($calc_array->operator == "0" || (!$is_single_operator && ($calc_array->target == "0" && !is_numeric($calc_array->value)))) && ($calc_array->target != "__open__" && $calc_array->target != "__close__"))	continue;
						...
						if ($use_target_value && !$use_custom_value && !isset($this->submission_data["form_elements"][$calc_array->target]) && $calc_array->target != "__open__" && $calc_array->target != "__close__") {
							$tmp_total = 0;
						...
						if ($use_target_value || $use_custom_value || $is_single_operator) {
							...
							if ($calc_array->target == "__close__") {
								if (!isset($element->calculate[$calc_array->reference_index])) continue;

								$target_value = $tmp_total;
								$tmp_total = $element->calculate[$calc_array->reference_index]->value;
							}
							else {
							...
							$target_value = $this->normalize_value($target_value, true);

							if ($calc_index == 0 && $element_type == "subtotal" && $calc_array->operator != "equals") {
							...
							switch ($calc_array->operator) {
								...
								case "equals":
									...
									if ($use_target_value) {
										if (property_exists($calc_array, "use_calculated_target_value")) {
											$target_element_data = json_decode($this->submission_data["form_elements"][$calc_array->target]->data);
				...
				if (property_exists($element, "add_to_price")) {
					if ($element->add_to_price == 1) {
						$total += $tmp_total;
					}
					else if ($element->add_to_price == 2) {
						$total = $tmp_total;
					}
				}

				// overwrite price
				if (property_exists($element, "overwrite_price") && $element->overwrite_price == 1 && !empty($element->add_to_price)) {
					$price_override = true;
					$total          = $tmp_total;
	...
	public function get_calculated_target_value_from_input($target_id, $input_value, $total_loop=0, $use_factor=true) {
	...
	public function get_raw_target_value($target_id, $input_value) {
		if (!isset($this->submission_data["form_elements"][$target_id])) return false;
	...
	**/
	public function get_total($data, $precision = null) {
		$total = 0;

		foreach ($data as $fe_id => $value) {
			// skip if element doesn't exist
			if (!isset($this->submission_data["form_elements"][$fe_id])) continue;
			...
			if (!empty($element->calculate_enabled)) {
				// add calculated value to price
				if (!empty($element->add_to_price)) {
					$total += $tmp_out["total"];
				}
				// overwrite price
				if (!empty($element->overwrite_price) && !empty($element->add_to_price)) {
					$total = $tmp_out["total"];
				}
			}
		}
	...
	**/
	public function get_output($id=null, $name=null, $product_id=null, $theme=null, $preview=null) {
		...
		foreach ($form_elements as $i => $element) {
			...
			$el_type       = !empty($data->extension) ? "extension" : $this->elements[$element->e_id]->type;
			$el_text       = "";
			$required      = "";
			$required_char = "";
			$step          = false;
			...
			// add name
			if (!empty($data->name)) {
				$data_add .= " data-elementname='" . esc_attr($data->name) . "'";
			}
			...
			if (property_exists($data, "calculate") && count($data->calculate) > 0) {
				$data_add .= " data-calculate_rows='" . esc_js(json_encode($data->calculate)) . "'";
			}
			...
			if (property_exists($data, "conditional") && count($data->conditional) > 0) {
				foreach ($data->conditional as $c => $conditional) {
					...
					if (property_exists($conditional, "operator_chain")) {
						...
						$data_conditional_output["compare_target_chain_{$c}"] = implode(",", (array) $compare_value);
			...
			if (property_exists($data, "discount") && count($data->discount) > 0) {
				foreach ($data->discount as $discount) {
					...
					$data_discount_output["discount_values"][] = property_exists($discount, "discount_value") ? $discount->discount_value : "";
				}
			...
			if (property_exists($data, "value")) {
				...
				if ($data->value == "__rand__" && property_exists($data, "min") && is_numeric($data->min) && property_exists($data, "max") && is_numeric($data->max)) {
					// a bit faster than rand()
					if (function_exists("mt_rand")) {
						$data->value = mt_rand($data->min, $data->max);
					}
					else {
						$data->value = rand($data->min, $data->max);
					}
				}
			}
			...
			// use custom GET-parameter value
				...
				// value over http(s) or any other protocol (via file_get_contents only)
					...
					// use curl instead
						...
						$curl = curl_init();
						curl_setopt($curl, CURLOPT_URL, $data->value_http);
					...
					if (!empty($data->value_http_json)) {
						...
						if (!$get_value_json) {
							...
							$json_keys = explode(".", $data->value_http_json);
			...
			switch ($el_type) {
				...
				case "numbers":
					...
					$el_text .= "<input	class='{$data->class} ezfc-element ezfc-element-numbers' {$data_factor}  id='{$el_child_id}' name='{$el_name}' placeholder='{$data->placeholder}' type='text' value='{$data->value}' data-settings='{$data_settings}' {$add_style} {$required} {$add_attr} />{$show_price}";
					$el_text .= $el_text_add;
				break;
				...
				case "dropdown":
					...
					// preselect value
					if (property_exists($data, "preselect")) $add_attr .= " data-initvalue='{$data->preselect}'";
					...
					foreach ($data->options as $n => $option) {
						...
						$el_text .= "<option value='{$n}' data-value='{$option->value}' data-index='{$n}' data-factor='{$option->value}' data-settings='{$data_settings}' {$add_data}>{$option->text}{$show_price}</option>";
					}
				...
				case "payment":
					...
					foreach ($data->options as $n => $radio) {
						...
						$el_text .= "	<input class='{$data->class} ezfc-element-radio-input' id='{$radio_id}' type='radio' name='{$el_name}' value='{$i}' data-value='{$radio->value}' data-index='{$i}' data-factor='{$radio->value}' data-settings='{$data_settings}' {$add_style} {$add_data}>";
						// image
						if (!empty($el_image)) {
							$el_image_style = "";
							$img_class = "";
				...
				case "checkbox":
					...
					foreach ($data->options as $n => $checkbox) {
						...
						$el_text .= "		<input class='{$data->class} ezfc-element-checkbox-input' id='{$checkbox_id}' type='checkbox' name='{$el_name_option}' value='{$i}' data-value='{$checkbox->value}' data-index='{$i}' data-factor='{$checkbox->value}' data-settings='{$data_settings}' {$add_style} {$add_data} />";
						// image
						if (!empty($el_image)) {
							$el_image_style = "";
				...
				case "datepicker":
					$data_settings = json_encode(array(
						...
						"available_days" => isset($data->available_days) ? $data->available_days : ""
					));
				...
				case "daterange":
					...
					// workdays only
					if (!empty($data->workdays_only)) $add_attr .= " data-workdays_only='{$data->workdays_only}'";
					...
					$el_text .= "<div class='ezfc-element-daterange-container' data-factor='{$data->factor}' {$add_attr}>";

					// from
					$el_text .= "<input class='{$data->class} ezfc-element ezfc-element-input ezfc-element-daterange ezfc-element-daterange-from' type='text' {$data_factor} name='{$el_name}[0]' {$data_value_external} data-mindays='{$data->minDays}' data-maxdays='{$data->maxDays}' placeholder='{$placeholder_values[0]}' data-mindate='{$minDate_values[0]}' data-maxdate='{$maxDate_values[0]}' {$add_style} {$required} {$add_attr_minDate} />";
					// to
					$el_text .= "<input class='{$data->class} ezfc-element ezfc-element-input ezfc-element-daterange ezfc-element-daterange-to' type='text' {$data_factor} name='{$el_name}[1]' {$data_value_external} data-mindays='{$data->minDays}' data-maxdays='{$data->maxDays}' placeholder='{$placeholder_values[1]}' data-mindate='{$minDate_values[1]}' data-maxdate='{$maxDate_values[1]}' {$add_style} {$required} {$add_attr_maxDate} />";
				...
				case "timepicker":
					wp_enqueue_style("ezfc-jquery-timepicker", plugins_url("assets/css/jquery.timepicker.css", __FILE__));
				...
				case "recaptcha":
					...
					$recaptcha_language = empty($data->language) ? "" : "?lang={$data->language}";
				...
				case "html":
					...
					// placeholders
					if (!empty($data->replace_placeholders)) {
						$tmp_text_placeholder = explode("{{", $tmp_text);
						if (count($tmp_text_placeholder > 0)) {
							foreach ($tmp_text_placeholder as $tmp_placeholder) {
								...
								if (count($placeholder_array) > 0) {
									...
									$placeholder_empty = property_exists($data, "placeholder_empty_text") ? $data->placeholder_empty_text : "";

									$placeholder_html = "<span class='ezfc-html-placeholder' data-listen_target='" . esc_js($placeholder) . "' data-empty='" . esc_js($placeholder_empty) . "'></span>";

									$tmp_text = str_replace("{{" . $placeholder . "}}", $placeholder_html, $tmp_text);
								}
							}
						}
					}
				...
				case "fileupload":
					...
					wp_enqueue_script("ezfc-jquery-iframe-transport", plugins_url("assets/js/jquery.iframe-transport.min.js", __FILE__), array("jquery-ui-widget"));
				...
				case "stepstart":
					$step_class = "ezfc-step ezfc-step-start {$data->wrapper_class}";
					if ($current_step == 0) $step_class .= " ezfc-step-active";

					$el_text = "<div class='{$step_class}' data-step='{$current_step}' data-id='{$element->id}' id='ezfc-element_{$element->id}'>";

					if (!empty($data->title) && $options["step_use_titles"] == 0) {
						$el_text .= "<div class='ezfc-step-title {$data->class}'>{$data->title}</div>";
					}
				...
				case "set":
					...
					$el_text .= "<input class='{$data->class} ezfc-element ezfc-element-input ezfc-element-set' type='text' name='{$el_name}' value='' {$add_style} data-settings='{$data_settings}' />";
				break;

				case "post":
					if ($data->post_id == 0) continue;
				...
				case "custom_calculation":
					$function_id = "ezfc_custom_calculation_" . substr(md5(microtime()), rand(0, 26), 10);

					// replace magic values
					$data->custom_calculation = str_replace("__form_id__", $form->id, $data->custom_calculation);
		...
		if ($options["show_price_position"] == 4 ||	$options["show_price_position"] == 5) {
			...
			$html .= "<div class='ezfc-fixed-price ezfc-fixed-price-{$fixed_price_position} ezfc-price-wrapper-element' data-id='{$form->id}'>";
	...
	public function insert_file($f_id, $ref_id, $file) {
		...
		return Ezfc_Functions::send_message("success", __("File entry saved.", "ezfc"));
	}
	...
	**/
	...
	/**
		normalize value
	**/
	public function normalize_value($value, $gracious = false) {
		$decimal_point = get_option("ezfc_email_price_format_dec_point", ".");

		// gracious normalize will only replace commas with dots to make sure the number can be parsed as float
		if ($gracious) {
	...
	**/
	...
	public function check_conditional_email_target($submission_data = null) {
		...
		foreach ($form_elements as $k => $fe) {
			...
			$email_target_array = Ezfc_conditional::check_conditional($fe_data, $element_value, "email_target", true);
