jQuery(document).ready(function($) {
	...
	**/
	function calculate_conditionals(form, form_id, el_object, el_type) {
		...
		// value of this element (but beware of is_currency due to text values)
		var el_value  = ezfc_get_value_from_element(el_object, null, !el_object.data("is_currency"));
		var el_factor = 1;
	...
	**/
	function calculate_elements(form, form_id, el_object, el_type, price) {
		...
		$(calc_list).each(function(cl, cl_object) {
			...
			var check_visible = $(el_object).is(":visible");

			// if we use steps, check_visible needs to be changed to ezfc-custom-hidden selector
			if (form_has_steps && !el_object.closest(".ezfc-step-active").length) {
				check_visible = !el_object.hasClass("ezfc-custom-hidden");
			}
