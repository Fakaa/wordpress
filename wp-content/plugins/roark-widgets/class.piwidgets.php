<?php
/*
Plugin Name: Roark Widgets
Author: nexttheme
Plugin URI: https://themeforest.net/user/next-theme
Author URI: https://themeforest.net/user/next-theme
Version: 1.0
Description: Roark Widgets
*/
...
if ( !class_exists('piWilokeWidgets') && class_exists('WP_Widget') )
{
    ...
    class piWilokeWidgets extends WP_Widget
    {
        ...
        public function __construct($widgetID="", $widgetName="", $aParams)
        {
            ...
            {
                parent::__construct($widgetID, $widgetName);
            }
        }
        ...
        public function pi_upload_field($label, $id, $buttonId, $name, $multiple=false, $value, $des="")
        {
            ...
            <p>
                ...
                <?php
                    if ( !empty($des) )
                    {
                        print '<code class="help">'. sprintf( (__('%s', 'wiloke')), wp_unslash($des) ) . '</code>';
                    }
                ?>
                        ...
                        <div class="image-cover">
                            <?php if ( has_post_thumbnail($query->post->ID) ) : ?>
                                <a href="<?php echo esc_url($link); ?>">
                                    <?php echo get_the_post_thumbnail($query->post->ID, 'thumbnail'); ?>
                                </a>
                            <?php endif; ?>
                        </div>
                    </div>
    ...
    {
        $aWidgets = array('piAbout', 'piFollow', 'piPostsListing', 'piMailchimp', 'piTWitterFeed', 'piContactInfo', 'piFlickrFeed', 'piInstagram', 'piAds', 'piFacebookLikebox');
